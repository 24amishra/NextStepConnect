...
interface ApplicationFormProps {
  business: PublicBusinessData;  // Changed from BusinessData
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
}

const ApplicationForm = ({ business, open, onOpenChange, onSuccess }: ApplicationFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (business.customQuestions) {
      for (const question of business.customQuestions) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-heading">Apply to {business.companyName}</DialogTitle>
          <DialogDescription>
            Submit your application below. Your information will be sent to the business.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 p-4 bg-secondary/50 rounded-lg border border-primary/20">
            ...
            <div className="text-sm text-muted-foreground">
              This information will be shared with {business.companyName}
            </div>
          </div>

          {/* Custom Questions */}
          {business.customQuestions && business.customQuestions.length > 0 && (
            <div className="space-y-4">
              <h3 className="font-semibold text-foreground">Additional Questions</h3>
              {business.customQuestions.map((question: CustomQuestion, index: number) => (
